...
{%- set source_number = loop.index | string -%}
row_rank_{{ source_number }}_{{ source_number }} AS (
    {%- if model.config.materialized == 'vault_insert_by_rank' %}
    SELECT {{ source_cols_with_rank | join(', ') }},
    {%- else %}
    SELECT {{ source_cols | join(', ') }},
    ...
    AND {{ dbtvault.multikey(fk_cols, condition='IS NOT NULL') }}
    {%- endif %}
    {%- set ns.last_cte = "row_rank_{}_{}".format(source_number, source_number) %}
),
row_rank_{{ source_number }} AS (
    {%- if model.config.materialized == 'vault_insert_by_rank' %}
    SELECT {{ source_cols_with_rank | join(', ') }},
...
row_rank_union_row_number AS (
    ...
    WHERE {{ src_pk }} IS NOT NULL
    AND {{ dbtvault.multikey(fk_cols, condition='IS NOT NULL') }}
    {%- set ns.last_cte = "row_rank_union_row_number" %}
),
row_rank_union AS (
    SELECT *
    FROM {{ ns.last_cte }}
