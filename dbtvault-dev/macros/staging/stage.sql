{%- macro stage(include_source_columns=none, source_model=none, hashed_columns=none, derived_columns=none, ranked_columns=none, selected_columns=none) -%}
...
{%- macro spark__stage(include_source_columns, source_model, hashed_columns, derived_columns, ranked_columns, selected_columns) -%}
...
{% if source_model is mapping and source_model is not none -%}
    ...
    {%- set source_relation = source(source_name, source_table_name) -%}
    {%- set all_source_columns = dbtvault.source_columns_from_selected(source_relation=source_relation, selected_columns=selected_columns) -%}
...
{%- set exclude_column_names = derived_column_names + hashed_column_names %}
{%- set source_and_derived_column_names = all_source_columns + derived_column_names %}
{%- set selected_columns_names = dbtvault.extract_column_names(selected_columns) -%}
...
WITH source_data AS (
    ...
    SELECT
    {{- "\n\n    " ~ dbtvault.print_list_from_selected(all_source_columns, selected_columns) if all_source_columns else " *" }}
...
derived_columns AS (
    ...
    {{ dbtvault.derive_columns(source_relation=source_relation, columns=derived_columns, selected_columns=selected_columns) | indent(4) }}
